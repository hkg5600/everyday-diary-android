...
class SplashActivityViewModel(private val userService: UserService) : BaseViewModel() {
    ...
    fun refreshToken(token: String) = addDisposable(userService.refreshToken(TokenRequest(token)), getDataObserver())

    fun insertToken(token: String, refreshToken: String) = insertToken(Token(1, token, refreshToken))
}